...
2. 首次配置
   - 若您是默认大学[JXUST],运行程序后输入校园网账号/密码
   - 若您是默认大学[JXUST],选择运营商（电信/移动/联通）【默认电信】
   - 若您不是默认IP,请进入【设置】-【认证设置】查看配置教程!!
   - 点击【保存配置】存储登录凭证

3. 功能操作
   - 启动脚本：点击【启动脚本】开始自动检测网络状态
   - 后台运行：通过【设置】-【后台常驻】启用托盘最小化功能,利于托管
   - 高级设置：
     * 调整检测周期（默认60秒）
     * 设置最大重连次数（默认10次）
     * 启用Wifi自动切换
...
1. 网络环境
   ...
   - 主界面的"电信""移动""联通"仅为适用[JXUST]的预设标识码
   - 因此其他学校可能不适配主界面的账号密码输入框和运营商通道
...
三、技术特性:
1. 采用maliang/tkinter实现多界面GUI兼容(可任一删除裁切)
